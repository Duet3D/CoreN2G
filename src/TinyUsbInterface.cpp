...
# endif
	...
	// Set up USB pins. All 3 fuses are in the same 32-bit word.
	static_assert(USB_FUSES_TRANSP_ADDR == USB_FUSES_TRANSN_ADDR);
	static_assert(USB_FUSES_TRIM_ADDR == USB_FUSES_TRANSN_ADDR);
	const uint32_t usbFuses = *reinterpret_cast<const uint32_t*>(USB_FUSES_TRANSN_ADDR);
	const uint32_t transpFuse = (usbFuses & USB_FUSES_TRANSP_Msk) >> USB_FUSES_TRANSP_Pos;
	const uint32_t transnFuse = (usbFuses & USB_FUSES_TRANSN_Msk) >> USB_FUSES_TRANSN_Pos;
	const uint32_t trimFuse = (usbFuses & USB_FUSES_TRIM_Msk) >> USB_FUSES_TRIM_Pos;
	USB->DEVICE.PADCAL.reg = USB_PADCAL_TRANSP(transpFuse)
						   | USB_PADCAL_TRANSN(transnFuse)
						   | USB_PADCAL_TRIM(trimFuse);
